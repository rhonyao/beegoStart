...
func (this *Client) Form(opts Options) string {
	...
	param.Extra_common_param = opts.Extra_common_param
	//生成签名
	sign := sign(param)
	...
	return `
		<form id="alipaysubmit" name="alipaysubmit" action="https://mapi.alipay.com/gateway.do?_input_charset=utf-8" method="get" style='display:none;'>
			...
			<input type="hidden" name="extra_common_param" value="` + param.Extra_common_param + `">
...
func (this *Client) Return(contro *beego.Controller) *Result {
	...
	type Params struct {
		...
		Extra_common_param    string `form:"extra_common_param" json:"extra_common_param"`       // 签名类型
	...
	if param.OutTradeNo == "" { //不存在交易号
		...
		if sign == param.Sign { //只有相同才说明该订单成功了
			...
			if param.TradeStatus == "TRADE_FINISHED" || param.TradeStatus == "TRADE_SUCCESS" { //交易成功
				...
				result.Extra_common_param = param.Extra_common_param
				result.TotalFee = param.TotalFee
				return result
			} else { // 交易未完成，返回错误代码-4
				result.Status = -4
...
func (this *Client) Notify(contro *beego.Controller) *Result {
	...
	postArray := strings.Split(string(contro.Ctx.Input.RequestBody), "&")
	...
	if paramSign == sign { // 传进的签名等于计算出的签名，说明请求合法
		...
		if urls.Get("trade_status") == "TRADE_FINISHED" || urls.Get("trade_status") == "TRADE_SUCCESS" { //交易成功
			...
			result.GmtPayment = urls.Get("gmt_payment")
			result.TotalFee = urls.Get("total_fee")
			result.Extra_common_param = urls.Get("extra_common_param")
			return result
		} else {
